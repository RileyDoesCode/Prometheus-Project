...
local function generateError(self, message)
	...
	return string.format("Parsing Error at Position %d:%d, %s", token.line, token.linePos, message)
end

local function generateWarning(token, message)
	return string.format("Warning at Position %d:%d, %s", token.line, token.linePos, message)
end
...
function Parser:statement(scope, currentLoop)
	...
	if consume(self, TokenKind.Keyword, "if") then
		...
		while consume(self, TokenKind.Keyword, "elseif") do
			local elif_condition = self:expression(scope)
			expect(self, TokenKind.Keyword, "then")
			local elif_body = self:block(scope, currentLoop)
	...
	if consume(self, TokenKind.Keyword, "local") then
		...
		-- Variables can only be referred to in the next statement, so ids are enabled after the expressions are parsed
	...
	if expr then
		...
		if expr.kind == AstKind.IndexExpression or expr.kind == AstKind.VariableExpression then
			...
			if self.luaVersion == LuaVersion.LuaU then
				...
				if consume(self, TokenKind.Symbol, "+=") then
					return Ast.CompoundAddStatement(expr, self:expression(scope))
				elseif consume(self, TokenKind.Symbol, "-=") then
					return Ast.CompoundSubStatement(expr, self:expression(scope))
				elseif consume(self, TokenKind.Symbol, "*=") then
					return Ast.CompoundMulStatement(expr, self:expression(scope))
				elseif consume(self, TokenKind.Symbol, "/=") then
					return Ast.CompoundDivStatement(expr, self:expression(scope))
				elseif consume(self, TokenKind.Symbol, "%=") then
					return Ast.CompoundModStatement(expr, self:expression(scope))
				elseif consume(self, TokenKind.Symbol, "^=") then
					return Ast.CompoundPowStatement(expr, self:expression(scope))
				elseif consume(self, TokenKind.Symbol, "..=") then
					return Ast.CompoundConcatStatement(expr, self:expression(scope))
				end
			end
			...
			while consume(self, TokenKind.Symbol, ",") do
				...
				if not expr then
					if self.disableLog then error() end
					logger:error(generateError(self, "expected a valid assignment statement lhs part but got nil"))
				end
...
function Parser:exprList(scope)
	local expressions = { self:expression(scope) }
	while consume(self, TokenKind.Symbol, ",") do
...
function Parser:expressionComparision(scope)
	...
	while true do
		if consume(self, TokenKind.Symbol, "<") then
			curr = Ast.LessThanExpression(curr, self:expressionStrCat(scope), true)
		elseif consume(self, TokenKind.Symbol, ">") then
			curr = Ast.GreaterThanExpression(curr, self:expressionStrCat(scope), true)
		elseif consume(self, TokenKind.Symbol, "<=") then
			curr = Ast.LessThanOrEqualsExpression(curr, self:expressionStrCat(scope), true)
		elseif consume(self, TokenKind.Symbol, ">=") then
			curr = Ast.GreaterThanOrEqualsExpression(curr, self:expressionStrCat(scope), true)
		elseif consume(self, TokenKind.Symbol, "~=") then
			curr = Ast.NotEqualsExpression(curr, self:expressionStrCat(scope), true)
		elseif consume(self, TokenKind.Symbol, "==") then
			curr = Ast.EqualsExpression(curr, self:expressionStrCat(scope), true)
		else
			break
		end
	end
...
function Parser:expressionAddSub(scope)
	...
	while true do
		if consume(self, TokenKind.Symbol, "+") then
			curr = Ast.AddExpression(curr, self:expressionMulDivMod(scope), true)
		elseif consume(self, TokenKind.Symbol, "-") then
			curr = Ast.SubExpression(curr, self:expressionMulDivMod(scope), true)
		else
			break
		end
	end
...
function Parser:expressionMulDivMod(scope)
	...
	while true do
		if consume(self, TokenKind.Symbol, "*") then
			curr = Ast.MulExpression(curr, self:expressionUnary(scope), true)
		elseif consume(self, TokenKind.Symbol, "/") then
			curr = Ast.DivExpression(curr, self:expressionUnary(scope), true)
		elseif consume(self, TokenKind.Symbol, "%") then
			curr = Ast.ModExpression(curr, self:expressionUnary(scope), true)
		else
			break
		end
	end
...
function Parser:expressionUnary(scope)
	if consume(self, TokenKind.Keyword, "not") then
		return Ast.NotExpression(self:expressionUnary(scope), true)
	end
	if consume(self, TokenKind.Symbol, "#") then
		return Ast.LenExpression(self:expressionUnary(scope), true)
	end
	if consume(self, TokenKind.Symbol, "-") then
		return Ast.NegateExpression(self:expressionUnary(scope), true)
	end
...
function Parser:expressionLiteral(scope)
	...
	if is(self, TokenKind.Ident) then
		local ident = get(self)
		local resolvedScope, id = scope:resolve(ident.value)
		return Ast.VariableExpression(resolvedScope, id)
	end

	-- IfElse (Bugfix applied here)
	if self.luaVersion == LuaVersion.LuaU then
		if consume(self, TokenKind.Keyword, "if") then
			local condition = self:expression(scope)
			expect(self, TokenKind.Keyword, "then")
	...
	if self.disableLog then error() end
	logger:error(generateError(self, "Unexpected Token \"" .. peek(self).source .. "\". Expected an Expression!"))
end
