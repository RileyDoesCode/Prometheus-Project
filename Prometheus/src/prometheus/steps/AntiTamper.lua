...
AntiTamper.SettingsDescriptor = {
	UseDebug = {
		type = "boolean",
		default = true,
		description = "Use debug library. (Recommended, however scripts will not work without the debug library.)"
	}
}
...
function AntiTamper:apply(ast, pipeline)
	...
	if self.UseDebug then
		local rndString = RandomStrings.randomString()
		code = code .. [[
			-- Anti Beautify
	...
	code = code .. [[
			...
			repeat until valid;
		end -- Safely closes the initial 'do' block
	]]

	local parsed = Parser:new({LuaVersion = Enums.LuaVersion.Lua51}):parse(code)
	
	-- We grab statement 1 (the single DoStatement that now correctly encapsulates everything)
