...
local function generateLocalMetatableInfo(pipeline)
	...
	info.valueName = nameGen(math.random(1, 4096))
...
function ProxifyLocals:apply(ast, pipeline)
	...
	-- Prepare Empty Function Variable (inserted only if used)
		...
		if node.kind == AstKind.AssignmentStatement then
			if #node.lhs == 1 and node.lhs[1].kind == AstKind.AssignmentVariable then
				...
				if localMetatableInfo then
					...
					-- Fallback to NilExpression if the RHS is empty/nil
					local rhsValue = args[1] or Ast.NilExpression()
					args[1] = localMetatableInfo.setValue.constructor(vexp, rhsValue)
	...
	-- Add Empty Function Declaration (Only if it was actually used)
	if self.emptyFunctionUsed then
