...
-- Cache compound operators to avoid recreating this table on every statement unparse
...
function Unparser:unparseStatement(statement, tabbing)
	...
	elseif self.luaVersion == LuaVersion.LuaU then
		local operator = COMPOUND_OPERATORS[kind]
		if operator then
			...
			logger:error(string.format("\"%s\" is not a valid unparseable statement in %s!", kind, self.luaVersion))
		end
	end
...
function Unparser:unparseExpression(expression, tabbing)
	...
	if kind == AstKind.NumberExpression then
		...
		if str == "inf" then return "2e1024" end
		if str == "-inf" then return "-2e1024" end
	...
	if kind == AstKind.VariableExpression or kind == AstKind.AssignmentVariable then
		return expression.scope:getVariableName(expression.id)
	end
	
	if kind == AstKind.StringExpression then
		return "\"" .. util.escape(expression.value) .. "\""
	end
	
	if kind == AstKind.NilExpression then return "nil" end
	if kind == AstKind.VarargExpression then return "..." end
	...
	if kind == AstKind.AndExpression then
		local lhs = self:unparseExpression(expression.lhs, tabbing)
		if Ast.astKindExpressionToNumber(expression.lhs.kind) >= Ast.astKindExpressionToNumber(kind) then
			lhs = "(" .. lhs .. ")"
		end
		
		local rhs = self:unparseExpression(expression.rhs, tabbing)
		if Ast.astKindExpressionToNumber(expression.rhs.kind) >= Ast.astKindExpressionToNumber(kind) then
			rhs = "(" .. rhs .. ")"
		end
	...
	if kind == AstKind.IndexExpression or kind == AstKind.AssignmentIndexing then
		...
		if expression.base.kind == AstKind.VarargExpression or Ast.astKindExpressionToNumber(expression.base.kind) > Ast.astKindExpressionToNumber(kind) then
			base = "(" .. base .. ")"
		end
	...
	logger:error(string.format("\"%s\" is not a valid unparseable expression", kind))
end
