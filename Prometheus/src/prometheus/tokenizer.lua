...
function Tokenizer:prepareGetPosition()
	...
	for index = 1, self.length do
		-- Using string.byte instead of string.sub to prevent massive string allocations
		local characterByte = string.byte(self.source, index) 

		local columnLength = column.length + 1
		column.length = columnLength
		column.charMap[index] = columnLength

		if characterByte == 10 then -- 10 is '\n'
...
function Tokenizer:new(settings) 
	...
	if conventions == nil then
		logger:error("The Lua Version \"" .. tostring(luaVersion) .. "\" is not recognised by the Tokenizer! Please use one of the following: \"" .. table.concat(keys(Tokenizer.Conventions), "\",\"") .. "\"")
	end
...
-- skip one or 0 Comments and return whether one was found
...
function Tokenizer:ident()
	local startPos = self.index
	expect(self, self.IdentCharsLookup)
	
	while is(self, self.IdentCharsLookup) do
		self.index = self.index + 1
	end
	
	local source = self.source:sub(startPos + 1, self.index)
...
function Tokenizer:multiLineString()
	...
	if is(self, "[") then
		...
		if is(self, "[") then
			...
			-- Highly optimized multiline string matching
			local endToken = "]" .. string.rep("=", eqCount) .. "]"
			local s, e = string.find(self.source, endToken, self.index + 1, true)
			
			if s then
				local value = string.sub(self.source, self.index + 1, s - 1)
				self.index = e
...
function Tokenizer:scanAll()
	...
	repeat
		local tok = self:next()
		table.insert(tb, tok)
	until tok.kind == Tokenizer.TokenKind.Eof
	return tb
end
