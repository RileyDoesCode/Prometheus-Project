...
function Compiler:createBlock()
	local id
	repeat
		id = math.random(0, 16777216) -- 2^24
...
function Compiler:compile(ast)
	...
	self.upvalsProxyLenReturn = math.random(-4194304, 4194304) -- 2^22
...
function Compiler:emitContainerFuncBody()
	...
	for _, block in ipairs(self.blocks) do
		...
		-- Shuffle Blockstats (Optimized shifting dependency check)
		...
		for _, stat in ipairs(blockstats) do
			table.insert(cleanStats, stat.statement)
		end

		table.insert(blocks, { id = id, block = Ast.Block(cleanStats, block.scope) })
	end
...
function Compiler:copyRegisters(scope, to, fromTb)
	local idStats = {}
	local vals    = {}
...
function Compiler:compileStatement(statement, funcDepth)
	...
	if statement.kind == AstKind.IfStatement then
		...
		for i, eif in ipairs(statement.elseifs) do
			...
			local eifInnerBlock = self:createBlock()
			if statement.elsebody or i < #statement.elseifs then
				nextBlock = self:createBlock()
			else
				nextBlock = finalBlock
			end
			local eifScope = self.activeBlock.scope
			self:addStatement(self:setRegister(eifScope, self.POS_REGISTER, Ast.OrExpression(Ast.AndExpression(self:register(eifScope, conditionReg), Ast.NumberExpression(eifInnerBlock.id)), Ast.NumberExpression(nextBlock.id))), {self.POS_REGISTER}, {conditionReg}, false)

			self:freeRegister(conditionReg, false)

			self:setActiveBlock(eifInnerBlock)
			eifScope = eifInnerBlock.scope
			self:compileBlock(eif.body, funcDepth)
			self:addStatement(self:setRegister(eifScope, self.POS_REGISTER, Ast.NumberExpression(finalBlock.id)), {self.POS_REGISTER}, {}, false)
		end
	...
	if statement.kind == AstKind.WhileStatement then
		...
		local checkScope = self.activeBlock.scope
		local conditionReg = self:compileExpression(statement.condition, funcDepth, 1)[1]
		self:addStatement(self:setRegister(checkScope, self.POS_REGISTER, Ast.OrExpression(Ast.AndExpression(self:register(checkScope, conditionReg), Ast.NumberExpression(innerBlock.id)), Ast.NumberExpression(finalBlock.id))), {self.POS_REGISTER}, {conditionReg}, false)
		self:freeRegister(conditionReg, false)

		self:setActiveBlock(innerBlock)
		local innerScope = self.activeBlock.scope
		self:compileBlock(statement.body, funcDepth)
		self:addStatement(self:setPos(innerScope, checkBlock.id), {self.POS_REGISTER}, {}, false)
		self:setActiveBlock(finalBlock)
		return
	end
	...
	if statement.kind == AstKind.RepeatStatement then
		...
		-- Bug fix: scope evaluation needed to bind to body's locals
		local evalScope = self.activeBlock.scope
		
		-- Evaluate condition
		local conditionReg = self:compileExpression(statement.condition, funcDepth, 1)[1]
		self:addStatement(self:setRegister(evalScope, self.POS_REGISTER, Ast.OrExpression(Ast.AndExpression(self:register(evalScope, conditionReg), Ast.NumberExpression(finalBlock.id)), Ast.NumberExpression(innerBlock.id))), { self.POS_REGISTER }, { conditionReg }, false)
		self:freeRegister(conditionReg, false)

		-- Clean up body's local variables
		for id in ipairs(statement.body.scope.variables) do
			local varReg = self:getVarRegister(statement.body.scope, id, funcDepth, nil)
			if self:isUpvalue(statement.body.scope, id) then
				evalScope:addReferenceToHigherScope(self.scope, self.freeUpvalueFunc)
				self:addStatement(self:setRegister(evalScope, varReg, Ast.FunctionCallExpression(Ast.VariableExpression(self.scope, self.freeUpvalueFunc), { self:register(evalScope, varReg) })), { varReg }, { varReg }, false)
			else
				self:addStatement(self:setRegister(evalScope, varReg, Ast.NilExpression()), { varReg }, {}, false)
			end
			self:freeRegister(varReg, true)
		end
	...
	if statement.kind == AstKind.ForStatement then
		...
		self:setActiveBlock(checkBlock)
		local checkScope = checkBlock.scope
		self:addStatement(self:setRegister(checkScope, currentReg, Ast.AddExpression(self:register(checkScope, currentReg), self:register(checkScope, incrementReg))), {currentReg}, {currentReg, incrementReg}, false)
		local tmpReg1 = self:allocRegister(false)
		local tmpReg2 = self:allocRegister(false)
		self:addStatement(self:setRegister(checkScope, tmpReg2, Ast.NotExpression(self:register(checkScope, incrementIsNegReg))), {tmpReg2}, {incrementIsNegReg}, false)
		self:addStatement(self:setRegister(checkScope, tmpReg1, Ast.LessThanOrEqualsExpression(self:register(checkScope, currentReg), self:register(checkScope, finalReg))), {tmpReg1}, {currentReg, finalReg}, false)
		self:addStatement(self:setRegister(checkScope, tmpReg1, Ast.AndExpression(self:register(checkScope, tmpReg2), self:register(checkScope, tmpReg1))), {tmpReg1}, {tmpReg1, tmpReg2}, false)
		self:addStatement(self:setRegister(checkScope, tmpReg2, Ast.GreaterThanOrEqualsExpression(self:register(checkScope, currentReg), self:register(checkScope, finalReg))), {tmpReg2}, {currentReg, finalReg}, false)
		self:addStatement(self:setRegister(checkScope, tmpReg2, Ast.AndExpression(self:register(checkScope, incrementIsNegReg), self:register(checkScope, tmpReg2))), {tmpReg2}, {tmpReg2, incrementIsNegReg}, false)
		self:addStatement(self:setRegister(checkScope, tmpReg1, Ast.OrExpression(self:register(checkScope, tmpReg2), self:register(checkScope, tmpReg1))), {tmpReg1}, {tmpReg1, tmpReg2}, false)
		self:freeRegister(tmpReg2)
		
		tmpReg2 = self:compileExpression(Ast.NumberExpression(innerBlock.id), funcDepth, 1)[1]
		self:addStatement(self:setRegister(checkScope, self.POS_REGISTER, Ast.AndExpression(self:register(checkScope, tmpReg1), self:register(checkScope, tmpReg2))), {self.POS_REGISTER}, {tmpReg1, tmpReg2}, false)
		self:freeRegister(tmpReg2)
		self:freeRegister(tmpReg1)
		
		tmpReg2 = self:compileExpression(Ast.NumberExpression(finalBlock.id), funcDepth, 1)[1]
		self:addStatement(self:setRegister(checkScope, self.POS_REGISTER, Ast.OrExpression(self:register(checkScope, self.POS_REGISTER), self:register(checkScope, tmpReg2))), {self.POS_REGISTER}, {self.POS_REGISTER, tmpReg2}, false)
		self:freeRegister(tmpReg2)

		self:setActiveBlock(innerBlock)
		local innerScope = innerBlock.scope
		self.registers[self.POS_REGISTER] = posState

		local varReg = self:getVarRegister(statement.scope, statement.id, funcDepth, nil)

		if self:isUpvalue(statement.scope, statement.id) then
			innerScope:addReferenceToHigherScope(self.scope, self.allocUpvalFunction)
			self:addStatement(self:setRegister(innerScope, varReg, Ast.FunctionCallExpression(Ast.VariableExpression(self.scope, self.allocUpvalFunction), {})), {varReg}, {}, false)
			self:addStatement(self:setUpvalueMember(innerScope, self:register(innerScope, varReg), self:register(innerScope, currentReg)), {}, {varReg, currentReg}, true)
		else
			self:addStatement(self:setRegister(innerScope, varReg, self:register(innerScope, currentReg)), {varReg}, {currentReg}, false)
		end
	...
	if statement.kind == AstKind.ForInStatement then
		...
		self:addStatement(Ast.AssignmentStatement({
			self:registerAssignment(checkScope, exprregs[3]),
			varRegs[2] and self:registerAssignment(checkScope, varRegs[2]),
		}, {
			Ast.FunctionCallExpression(self:register(checkScope, exprregs[1]), {
				self:register(checkScope, exprregs[2]),
				self:register(checkScope, exprregs[3]),
			})
		}), {exprregs[3], varRegs[2]}, {exprregs[1], exprregs[2], exprregs[3]}, true)

		self:addStatement(Ast.AssignmentStatement({
			self:posAssignment(checkScope)
		}, {
			Ast.OrExpression(Ast.AndExpression(self:register(checkScope, exprregs[3]), Ast.NumberExpression(bodyBlock.id)), Ast.NumberExpression(finalBlock.id))
		}), {self.POS_REGISTER}, {exprregs[3]}, false)

		self:setActiveBlock(bodyBlock)
		local bodyScope = self.activeBlock.scope
		self:addStatement(self:copyRegisters(bodyScope, {varRegs[1]}, {exprregs[3]}), {varRegs[1]}, {exprregs[3]}, false)
		for i = 3, #varRegs do
			self:addStatement(self:setRegister(bodyScope, varRegs[i], Ast.NilExpression()), {varRegs[i]}, {}, false)
		end
		...
		for i, id in ipairs(statement.ids) do
			if self:isUpvalue(statement.scope, id) then
				...
				local tmpReg = self:allocRegister(false)
				bodyScope:addReferenceToHigherScope(self.scope, self.allocUpvalFunction)
				self:addStatement(self:setRegister(bodyScope, tmpReg, Ast.FunctionCallExpression(Ast.VariableExpression(self.scope, self.allocUpvalFunction), {})), {tmpReg}, {}, false)
				self:addStatement(self:setUpvalueMember(bodyScope, self:register(bodyScope, tmpReg), self:register(bodyScope, varreg)), {}, {tmpReg, varreg}, true)
				self:addStatement(self:copyRegisters(bodyScope, {varreg}, {tmpReg}), {varreg}, {tmpReg}, false)
				self:freeRegister(tmpReg, false)
			end
		end
	...
	if statement.kind == AstKind.ContinueStatement then
		...
		repeat
			statScope = statScope and statScope.parentScope or statement.scope
			for id in pairs(statScope.variables) do
				table.insert(toFreeVars, {
					scope = statScope,
					id = id,
				})
...
function Compiler:compileExpression(expression, funcDepth, numReturns)
	...
	if expression.kind == AstKind.OrExpression then
		...
		do
			...
			local block1Scope = block1.scope
			local rhsReg = self:compileExpression(expression.rhs, funcDepth, 1)[1]
			self:addStatement(self:copyRegisters(block1Scope, {resReg}, {rhsReg}), {resReg}, {rhsReg}, false)
			self:freeRegister(rhsReg, false)
			self:addStatement(self:setRegister(block1Scope, self.POS_REGISTER, Ast.NumberExpression(block2.id)), {self.POS_REGISTER}, {}, false)
		end

		self.registers[self.POS_REGISTER] = posState

		self:setActiveBlock(block2)
		local block2Scope = block2.scope

		if tmpReg then
			self:addStatement(self:copyRegisters(block2Scope, {self.POS_REGISTER}, {tmpReg}), {self.POS_REGISTER}, {tmpReg}, false)
			self:freeRegister(tmpReg, false)
		end
			...
			if i == 1 then
				...
				for j, entry in ipairs(expression.entries) do
					if entry.kind == AstKind.TableEntry then
						...
						if j == #expression.entries and (value.kind == AstKind.FunctionCallExpression or value.kind == AstKind.PassSelfFunctionCallExpression or value.kind == AstKind.VarargExpression) then
	...
	logger:error(string.format("%s is not a compliable expression!", expression.kind))
end
