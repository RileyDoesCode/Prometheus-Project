...
function Pipeline:new(settings)
	...
	if not conventions then
		logger:error("The Lua Version \"" .. tostring(luaVersion) 
...
function Pipeline:fromConfig(cfg)
	...
	pipeline:setNameGenerator(cfg.NameGenerator or "MangledShuffled")
...
function Pipeline:setOption(name, value)
	assert(false, "TODO: Implement Pipeline:setOption")
