...
function Scope:addChild(scope)
	...
	for targetScope, ids in pairs(scope.variablesFromHigherScopes) do
		for id, count in pairs(ids) do
			if count and count > 0 then
				self:addReferenceToHigherScope(targetScope, id, count)
			end
		end
	end
...
function Scope:removeChild(child)
	for i, v in ipairs(self.children) do
		if v == child then
			-- This will add all References from that Scope to higher Scopes. Note that the higher scopes may only be global
			for targetScope, ids in pairs(v.variablesFromHigherScopes) do
				for id, count in pairs(ids) do
					if count and count > 0 then
						self:removeReferenceToHigherScope(targetScope, id, count)
					end
				end
			end
...
function Scope:renameVariables(settings)
	if not self.isGlobal then
		...
		for _, keyword in ipairs(settings.Keywords) do
	...
	for _, scope in ipairs(self.children) do
